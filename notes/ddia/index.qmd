...
## Summary
Handle conflicts can be hard because writes are *concurrent* so their is no natural order them. An arbitrary order needs to be applied to determine the last write when using the *last write wins* approach for conflict resolution. 